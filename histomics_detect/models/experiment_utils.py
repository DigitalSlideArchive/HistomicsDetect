...
def save_history(
    ...
    experiment_id: int,
    history_path: str = "JsonLogs/logs_{}.json",
    weight_path: str = "lnms_weights/cpk_{}",
) -> None:
...
def validate_model(
    ds_validation_roi: tf.data.Dataset,
    model: LearningNMS,
    faster_model,
    experiment_id: int = 0,
    ...
    for data in ds_validation_roi:
        ...
        if tf.size(filtered_boxes) > 0:
            try:
                rpn_boxes_final = faster_model.align(
                    filtered_boxes, features, faster_model.field, faster_model.pool, faster_model.tiles
                )

                boxes = filter_edge_boxes(
                    boxes, tf.shape(rgb)[1], tf.shape(rgb)[0], 32, tf.constant(False, tf.bool)
                )

                rpn_boxes_final, condition = filter_edge_boxes(
                    rpn_boxes_final, tf.shape(rgb)[1], tf.shape(rgb)[0], 32, tf.constant(False, tf.bool)
                )
...
def run_experiments(
    ...
    changing_variable: str,
    values: List[Union[str, tf.keras.losses.Loss, int, float, tf.Tensor]],
    experiment_start_id: int = 0,
    epochs: int = 100,
    steps_per_epoch: int = 92,
) -> None:
    ...
    for value in values:
        ...
        compression_net = CompressionNetwork(
            temp_configs["feature_size"], temp_configs["anchor_size"], faster_model.backbone
        )
