...
from histomics_detect.metrics import (
    iou,
    greedy_iou_mapping,
    AveragePrecision,
    FalsePositiveRate,
...
def map_outputs(output, anchors, anchor_px, field):
    ...
    index = tf.equal(
        tf.expand_dims(anchors[:, 3], axis=1), tf.expand_dims(tf.cast(anchor_px, tf.float32), axis=0)
    )
    ...
    reshaped = tf.reshape(
        output,
        tf.concat(
            [tf.shape(output)[0:-1], [tf.size(anchor_px)], [tf.shape(output)[-1] / tf.size(anchor_px)]],
            axis=0,
        ),
    )
    ...
    mapped = tf.gather_nd(reshaped, tf.stack([tf.zeros(tf.shape(px), tf.int32), py, px, index], axis=1))
...
class FasterRCNN(tf.keras.Model):
    ...
    def __init__(
        self, backbone_args, rpn_args, frcnn_args, train_args, validation_args, anchor_sizes, **kwargs
    ):
        ...
        self.anchors = create_anchors(
            self.anchor_px, self.field, train_args["train_shape"][0], train_args["train_shape"][1]
        )

        # define metrics
        self.objectness_metrics = [
        ...
        self.regression_metrics = [
            AveragePrecision(iou_thresh=t, delta=validation_args["ap_delta"], name="ap" + str(int(100 * t)))
    ...
    def input_size(self, size=[None, None]):
        ...
        if all([dim is None for dim in size]):
            ...
            rpnetwork_size = [
                tf.cast(tf.math.ceil(size[0] / self.field), tf.int32),
    ...
    def nms(self, boxes, objectness, nms_iou):
        ...
        selected = tf.image.non_max_suppression(
            tf_box_transform(boxes), objectness[:, 1], tf.shape(objectness)[0], iou_threshold=nms_iou
        )
    ...
    def call(self, rgb, tau=None, nms_iou=None, margin=0):
        ...
        if tau is None:
            tau = self.tau
        rpn_boxes_positive, rpn_obj_positive, positive = self.threshold(rpn_boxes, rpn_obj, tau)

        # perform non-max suppression on rpn positive predictions
        if nms_iou is None:
            nms_iou = self.nms_iou
        rpn_boxes_nms, rpn_obj_nms, selected = self.nms(rpn_boxes_positive, rpn_obj_positive, nms_iou)

        # generate roialign predictions for rpn positive predictions
        align_boxes = self.align(rpn_boxes_nms, features, self.field, self.pool, self.tiles)

        # filter edge boxes
        filtered, mask = filter_edge_boxes(align_boxes, tf.shape(rgb)[1], tf.shape(rgb)[0], margin)
    ...
    def test_step(self, data):
        ...
        anchors = create_anchors(self.anchor_px, self.field, tf.shape(rgb)[1], tf.shape(rgb)[0])
        ...
        positive_obj = tf.nn.softmax(map_outputs(output, positive_anchors, self.anchor_px, self.field))
        negative_obj = tf.nn.softmax(map_outputs(output, negative_anchors, self.anchor_px, self.field))
        obj_labels = tf.concat(
            [tf.ones(tf.shape(positive_obj)[0], tf.uint8), tf.zeros(tf.shape(negative_obj)[0], tf.uint8)],
            axis=0,
        )
        ...
        rpn_boxes_nms, rpn_obj_nms, selected = self.nms(rpn_boxes_pred, rpn_obj_pred, self.nms_iou)
        ...
        # filter edge boxes
        filtered, mask = filter_edge_boxes(align_boxes_nms, tf.shape(rgb)[1], tf.shape(rgb)[0], self.margin)
        ...
        obj_metrics = self._update_objectness_metrics(
            tf.concat([positive_obj, negative_obj], axis=0), obj_labels
        )

        # update regression metrics
        reg_metrics = self._update_regression_metrics(
            boxes, tf.concat([filtered_objectness, filtered], axis=1)
        )
        ...
        # dummy losses
        losses = {"loss_rpn_obj": 0.0, "loss_rpn_reg": 0.0, "loss_align_reg": 0.0}
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            positive_obj = tf.nn.softmax(map_outputs(output[0], positive_anchors, self.anchor_px, self.field))
            negative_obj = tf.nn.softmax(map_outputs(output[0], negative_anchors, self.anchor_px, self.field))

            # sample anchors and perform hard-negative mining
            positive_anchors, negative_anchors = sample_anchors(
                positive_anchors,
                negative_anchors,
                negative_obj[:, 1],
                self.max_anchors,
            ...
            obj_loss = self.loss[0](
                tf.concat([positive_obj, negative_obj], axis=0), tf.one_hot(obj_labels, 2)
            )

            # ccalculate rpn regression loss
            regression = map_outputs(output[1], positive_anchors, self.anchor_px, self.field)
            ...
            rpn_loss = obj_loss / self.max_anchors + reg_loss * self.lmbda / tf.cast(
                tf.shape(self.anchors)[0], tf.float32
            )
            ...
            def nonempty():
                ...
                interpolated = roialign(
                    features, rpn_boxes_positive, self.field, pool=self.pool, tiles=self.tiles
                )
        ...
        # update metrics
        metrics = self._update_objectness_metrics(tf.concat([positive_obj, negative_obj], axis=0), obj_labels)

        # build output loss dict
        losses = {"loss_rpn_obj": obj_loss, "loss_rpn_reg": reg_loss, "loss_align_reg": align_loss}
