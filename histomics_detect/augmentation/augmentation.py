...
def jitter_boxes(boxes, max_offset_percentage: float = 0.1, application_type: str = 'all'):
    """Randomly adds uniform noise (relative to box size) to boxes

    This function is used for data augmentation and adds uniform noise to the boxes.
    The noise is generated by uniformly sampling from -'max_offset_percentage' to 'max_offset_percentage'
    and multiplying with the boxes

    Parameters
    ----------
    application_type: string
        determines which of the box parameters should be jittered
        currently supported types:
        - 'all': all parameters are jittered
        - 'w': the width is jittered
        - 'h': the height is jittered
        - 'x': the x position is jittered
        - 'y': the y position is jittered

        can also be combined 'wh' jitters the height and the width ...
        the order does not matter
    max_offset_percentage: float
        max percentage of the boxes size that can contribute to the noise
    boxes: tensor (float32)
        M x 4 tensor where each row contains the x,y location of the upper left
        corner of a ground truth box and its width and height in that order.
    ...
    -------
    boxes: tensor (float32)
        N x 4 tensor with added noise
    """
    if max_offset_percentage >= 1 or max_offset_percentage <= -1:
        warnings.warn("'max_offset_percentage very high. This can lead to boxes being 0px large or even having negative"
                      "width of heights", UserWarning)

    boxes = tf.cast(boxes.to_tensor(), tf.float32)

    if application_type == 'all':
        mask = tf.ones(tf.shape(boxes))
    else:
        num_boxes = tf.shape(boxes)[0]
        x, y, w, h = tf.zeros((num_boxes, 1), tf.float32), tf.zeros((num_boxes, 1), tf.float32), \
                     tf.zeros((num_boxes, 1), tf.float32), tf.zeros((num_boxes, 1), tf.float32)
        if 'x' in application_type:
            x = tf.ones((num_boxes, 1))
        if 'y' in application_type:
            y = tf.ones((num_boxes, 1))
        if 'w' in application_type:
            w = tf.ones((num_boxes, 1))
        if 'h' in application_type:
            h = tf.ones((num_boxes, 1))
        mask = tf.concat((x, y, w, h), axis=1)

    offset_percentages = tf.random.uniform(tf.shape(boxes), -max_offset_percentage, max_offset_percentage, tf.float32)

    jittered_boxes = boxes + offset_percentages*tf.concat((boxes[:, 2:], boxes[:, 2:]), axis=1)*mask

    jittered_boxes = tf.RaggedTensor.from_tensor(jittered_boxes)
