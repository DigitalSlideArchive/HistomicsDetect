...
class FalseNegativeRate(tf.keras.metrics.Metric):
    ...
    def __init__(self, thresholds=None, name="fnr", **kwargs):
        ...
        self.false_negatives = self.add_weight(
            name="false_negatives", initializer="zeros", shape=(tf.size(self.thresholds),)
        )
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        binary = tf.less_equal(tf.reshape(y_pred, (tf.size(y_pred), 1)), self.thresholds)
        positives = tf.equal(tf.cast(y_true, tf.bool), True)
        false_negatives = tf.logical_and(tf.reshape(positives, (tf.size(y_true), 1)), binary)
    ...
    def result(self):
        return tf.cond(
            tf.equal(tf.reduce_sum(self.positives), 0.0),
            lambda: 0.0,
...
class FalsePositiveRate(tf.keras.metrics.Metric):
    ...
    def __init__(self, thresholds=None, name="fpr", **kwargs):
        ...
        self.false_positives = self.add_weight(
            name="false_positives", initializer="zeros", shape=(tf.size(self.thresholds),)
        )
    ...
    def reset_state(self):
        ...
        self.negatives.assign(0.0)

    def update_state(self, y_true, y_pred, sample_weight=None):

        # generate binary matrix of false positives
        binary = tf.greater(tf.reshape(y_pred, (tf.size(y_pred), 1)), self.thresholds)
        negatives = tf.equal(tf.cast(y_true, tf.bool), False)
        false_positives = tf.logical_and(tf.reshape(negatives, (tf.size(y_true), 1)), binary)
    ...
    def result(self):
        return tf.cond(
            tf.equal(tf.reduce_sum(self.negatives), 0.0),
            lambda: 0.0,
